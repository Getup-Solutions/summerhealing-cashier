<template>
    ...
    <Modal :modalHeadingText="'Edit Course'" :modalHeadingResetButton="true" :modalWidth="2">
                ...
                <div id="accordion-flush-body-2" class="hidden" aria-labelledby="accordion-flush-heading-2">
                    <div class="py-5 border-b border-gray-200 dark:border-gray-700">
                        <ul class="grid w-full gap-6 md:grid-cols-2">
                            <li v-for="subscription in subscriptionsPrices" :key="subscription">
        ...
        <template #footer>
            <Button
                @click.prevent="editRequest({ url: '/admin/dashboard/courses/', data: { ...courseInfo, subscriptionsPrices: selectedSubscriptionsWithPrice },dataId: course.id, only: ['flash', 'errors'] })"
...
export default {
props: ["errors","course", "agegroups", "levels", "trainers", "subscriptionsPrices","subscriptionsSelected","trainersSelected"],
data() {
    return {
        courseInfo: Object.assign(this.course, {subscriptionsSelected: this.subscriptionsSelected, trainers: this.trainersSelected}),
        deleteId: false,
        subscriptionPricing: this.course.selectedSubscriptionsWithPrice
    };
},
computed: {
        selectedSubscriptionsWithPrice() {
            var selectedSubscription = this.subscriptionsPrices.filter((item) => this.courseInfo.subscriptionsSelected.includes(item.id));
            // var newArr = selectedSubscription.map(v => (v.price = (v.price===0) ? ))
