<template>
    ...
    <DeleteAlert v-if="deleteId" :id="deleteId" @close="deleteId = false" :url="'/admin/dashboard/users/'"
        :text="'Deleting the trainer will permanently removed from the database. You can\'t recover the user again. Are you sure about deleting?'">
    </DeleteAlert>
    <TableLayout>
        <Filters :searchPlaceHolder="'Search by Trainer ID, Name, Email ..'" :filters="filters"
            :currentPage="users.current_page" :dataName="'trainers'" :sortByFilters="{ dateSort: true }" :enableFilters="{
                search: true,
                dateRange: true,
                sortBy: true,
