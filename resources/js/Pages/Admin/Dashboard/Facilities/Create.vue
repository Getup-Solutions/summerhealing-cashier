<template>
    ...
    <Modal :modalHeadingText="'Create new Facility'" :modalHeadingResetButton="true" :modalWidth="2">
        ...
        <template #footer>
            <Button
                @click.prevent="createRequest({ url: '/admin/dashboard/facilities', data: { ...facilityInfo, subscriptionplansPrices: selectedSubscriptionplansWithPrice }, only: ['flash', 'errors'] })"
...
export default {
    props: ["errors","trainers", "subscriptionplans"],
    data() {
        return {
            facilityInfo: { trainers: [], subscriptionplansSelected: [] },
            subscriptionPricing: []
        };
    },
    computed: {
        selectedSubscriptionplansWithPrice() {
            var selectedSubscription = this.subscriptionplans.filter((item) => this.facilityInfo.subscriptionplansSelected.includes(item.id));
            var newArr = selectedSubscription.map(v => ({ ...v, price: this.facilityInfo.price }))
            return newArr
        },
