    ...
    <Modal :modalHeadingText="Object.keys(url_params).length > 0 ? 'Create User' : 'Create User'"
        ...
        <template #body>
            ...
            <div v-if="userInfo.roles.length===0">
                ...
                <h5 class="mb-5 text-base font-medium text-gray-900 dark:text-white">Select Subscription(s):</h5>
                <ul class="grid w-full gap-6 md:grid-cols-2">
                    <li v-for="subscription in subscriptions" :key="subscription">
                        <input type="checkbox" :id="`react-option-${subscription.id}`" class="hidden peer" :value="subscription.id" v-model="userInfo.subscriptions"
                            required="">
                        <label :for="`react-option-${subscription.id}`"
                            ...
                            <div class="block bg-white/80 dark:bg-black/70 rounded p-4 w-full">
                                ...
                                <div class="w-full text-xs"><span class="font-semibold">Validity :</span> {{ subscription.validity }} days</div>
                                <div class="w-full text-xs"><span class="font-semibold">Price :</span>  {{ subscription.price }} AUD</div>
                            </div>
                        </label>
                    </li>
                </ul>
...
export default {
    props: ["errors", "roles", "url_params", "subscriptions"],
    data() {
        return {
            userInfo: {
