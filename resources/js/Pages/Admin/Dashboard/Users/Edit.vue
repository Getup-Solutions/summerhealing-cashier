    ...
    <Modal :modalHeadingText="'Edit User'" :modalHeadingResetButton="true" :modalWidth="2">
            ...
            <div v-if="userInfo.roles.length < 2">
                ...
                <ul class="grid w-full gap-6 md:grid-cols-2">
                    <li v-for="subscriptionplan in subscriptionplans" :key="subscriptionplan">
                        <input type="checkbox" :id="`react-option-${subscriptionplan.id}`" class="hidden peer"
                            :value="subscriptionplan.id" v-model="userInfo.subscriptionplans" required="">
                        <label :for="`react-option-${subscriptionplan.id}`"
                            ...
                            <div class="block bg-white/80 dark:bg-black/70 rounded p-4 w-full">
                                ...
                                <div class="w-full text-xs"><span class="font-semibold">Price :</span> {{ subscriptionplan.price
                                }} AUD</div>
                            </div>
                        </label>
                    </li>
        ...
        <template #footer>
            <Button @click.prevent="
                ...
                " :text="'Update User'" :color="'blue'"></Button>

            <Button @click.prevent="deleteId = user.id" :text="'Delete User'" :color="'red'"></Button>
...
<script>
export default {
    props: ["errors", "roles", "user", "user_roles", "user_subscriptionplans", "subscriptionplans"],
    data() {
        return {
            userInfo: Object.assign({ roles: this.user_roles, subscriptionplans: this.user_subscriptionplans }, this.user),
            deleteId: false,
        };
    },
    mounted() {
