            ...
            <div v-if="userInfo.roles.length < 2">
                ...
                <ul class="grid w-full gap-6 md:grid-cols-2">
                    <li v-for="subscription in subscriptions" :key="subscription">
                        <input type="checkbox" :id="`react-option-${subscription.id}`" class="hidden peer"
                            :value="subscription.id" v-model="userInfo.subscriptions" required="">
                        <label :for="`react-option-${subscription.id}`"
                            ...
                            <div class="block bg-white/80 dark:bg-black/70 rounded p-4 w-full">
                                ...
                                <div class="w-full text-xs"><span class="font-semibold">Price :</span> {{ subscription.price
                                }} AUD</div>
                            </div>
                        </label>
                    </li>
...
<script>
export default {
    props: ["errors", "roles", "user", "user_roles", "user_subscriptions", "subscriptions"],
    data() {
        return {
            userInfo: Object.assign({ roles: this.user_roles, subscriptions: this.user_subscriptions }, this.user),
            deleteId: false,
        };
    },
    mounted() {
