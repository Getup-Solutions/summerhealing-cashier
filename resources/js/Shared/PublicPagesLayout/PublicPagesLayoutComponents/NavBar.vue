<template>
          ...
          <DialogPanel class="relative flex w-full max-w-xs flex-col overflow-y-auto dark:bg-sh_dark_blue pb-12 shadow-xl">
            ...
            <TabGroup as="div" class="mt-2">
                  ...
                  <Tab as="template" v-for="subscription in navMenu.subscriptions" :key="subscription.name" v-slot="{ selected }">
                    <button :class="[
                      selected
                        ...
                        : 'border-transparent text-sh_yellow',
              ...
              <TabPanels as="template">
                <TabPanel v-for="subscription in navMenu.subscriptions" :key="subscription.name"
                  class="space-y-10 px-4 pb-8 pt-10 z-50">
                  <div class="grid grid-cols-2 gap-x-4">
                    <div v-for="item in subscription.featured" :key="item.name" class="group relative text-sm">
                  ...
                  <div v-for="section in subscription.sections" :key="section.name">
                    <p :id="`${subscription.id}-${section.id}-heading-mobile`" class="font-medium text-gray-900">
                      {{ section.name }}
                    </p>
                    <ul role="list" :aria-labelledby="`${subscription.id}-${section.id}-heading-mobile`"
                      class="mt-6 flex flex-col space-y-6">
                      <li v-for="item in section.items" :key="item.title" @click="open = false" class="flow-root">
                        <Link :href="item.slug" class="-m-2 block p-2 text-gray-500" preser>{{ item.title }}</Link>
                      </li>
                    </ul>
                  </div>
                </TabPanel>
            ...
            <div class="space-y-6 border-t border-gray-200 px-4 py-6">
              <div v-for="page in navMenu.pages" :key="page.name" class="flow-root">
                <Link :href="page.href" class="-m-2 block p-2 font-medium text-gray-300">{{ page.name }}</Link>
              </div>
            </div>

            <div class="space-y-6 border-t border-gray-200 px-4 py-6">
              <div class="flow-root">
                <a href="#" class="-m-2 block p-2 font-medium text-gray-300">Sign in</a>
              </div>
              <div class="flow-root">
                <a href="#" class="-m-2 block p-2 font-medium text-gray-300">Create account</a>
              </div>
            </div>
  ...
  <header class="relative bg-white dark:bg-sh_dark_blue">
    ...
    <nav aria-label="Top" class="mx-auto max-w-7xl px-4 sm:px-3 lg:px-4">
      <div class="border-b border-gray-200 dark:border-sh_dark_blue">
        <div class="flex h-20 items-center">
          <button type="button" class="rounded-md bg-sh_dark_blue p-2 text-gray-400 lg:hidden" @click="open = true">
          ...
          <div class="ml-4 flex lg:ml-0">
            ...
            <img class=" h-24 p-2 w-auto" :src="navMenu.logo.imageSrc" alt="" />
            </Link>
          </div>
              ...
              <Popover v-for="subscription in navMenu.subscriptions" :key="subscription.name" class="flex" v-slot="{ open }">
                <div class="relative flex">
                  <PopoverButton :class="[
                    open
                      ...
                      : 'border-transparent text-gray-700 dark:text-sh_yellow hover:text-gray-800',
                    'relative z-10 -mb-px flex items-center border-b-2 py-2 px-2 text-sm font-medium transition-colors duration-200 ease-out',
                  ]">{{ subscription.name }}</PopoverButton>
                </div>
                ...
                <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
                  ...
                  <PopoverPanel class="absolute inset-x-0 top-full text-sm text-gray-500" v-slot="{ close }">
                    ...
                    <div class="absolute inset-0 top-1/2 bg-white dark:bg-sh_dark_blue shadow" aria-hidden="true" />

                    <div class="relative bg-white dark:bg-sh_dark_blue">
          ...
          <div class="ml-auto flex items-center">
            ...
            <!-- <div class="hidden lg:ml-8 lg:flex">
              <a href="#" class="flex items-center text-gray-700 dark:text-sh_yellow hover:text-gray-800">
